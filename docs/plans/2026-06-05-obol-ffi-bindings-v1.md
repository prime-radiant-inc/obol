...
required). The `include_bytes!("../../../bindings/testdata/prices.json")` path is relative to
`crates/obol-ffi/src/lib.rs` → repo-root `bindings/testdata/prices.json`.
...
- [ ] **Step 2: Write `scripts/gen-header.sh`:**

```bash
#!/usr/bin/env bash
set -euo pipefail
cd "$(dirname "$0")/.."
mise exec rust@1.96.0 -- cbindgen --config crates/obol-ffi/cbindgen.toml \
  --crate obol-ffi --output crates/obol-ffi/include/obol.h
echo "wrote crates/obol-ffi/include/obol.h"
```

Make it executable: `chmod +x scripts/gen-header.sh`.

- [ ] **Step 3: Generate the header.** Ensure the cbindgen CLI is available
  (`mise exec rust@1.96.0 -- cargo install cbindgen --version ^0.27` if needed, or use the
  library path in Step 5). Run `./scripts/gen-header.sh` and open
...
- [ ] **Step 4: Write the drift test** in the `tests` module of `crates/obol-ffi/src/lib.rs`.
  It regenerates the header via the cbindgen **library** using the same toml, and asserts
  byte-equality with the committed header:
...
- [ ] **Step 5: Run the drift test, expect PASS.**
  `mise exec rust@1.96.0 -- cargo test -p obol-ffi header_matches_source -- --test-threads=1`.
  If it fails on formatting, regenerate with the script (the script uses the CLI; the test uses
  the library — if they differ in output, prefer making the committed header match the
  **library** output by writing it from a tiny helper, since the test is the gate. Adjust
  `gen-header.sh` to shell out to the same version, or commit the library-generated bytes.)
...
```bash
git add crates/obol-ffi/cbindgen.toml scripts/gen-header.sh crates/obol-ffi/include/obol.h crates/obol-ffi/src/lib.rs
git commit -m "feat(ffi): cbindgen header generation + drift test"
```
...
- [ ] **Step 5: Run the Python tests.** From `bindings/python`:

```bash
cd bindings/python && OBOL_LIB="$(git -C "$(pwd)" rev-parse --show-toplevel)/target/debug/$( [ "$(uname)" = Darwin ] && echo libobol_ffi.dylib || echo libobol_ffi.so )" PYTHONPATH=. python -m pytest tests -q
```

Expected: 5 passed. (The `OBOL_LIB` export is belt-and-suspenders; the loader's target/ fallback
also finds it.)
...
#cgo LDFLAGS: -L${SRCDIR}/../../../target/debug -lobol_ffi
#include <stdlib.h>
#include "obol.h"
*/
import "C"
...
- [ ] **Step 5: Run the Go tests** (set the runtime lib path so the loader finds the dylib):

```bash
cd bindings/go && \
  LIBDIR="$(cd ../../target/debug && pwd)" && \
  DYLD_LIBRARY_PATH="$LIBDIR" LD_LIBRARY_PATH="$LIBDIR" CGO_ENABLED=1 go test ./... 
```

Expected: ok, all tests pass. If the linker can't find `-lobol_ffi`, confirm
`target/debug/libobol_ffi.{dylib,so}` exists (`cargo build -p obol-ffi`).

- [ ] **Step 6: Write `bindings/go/README.md`** — usage, the `#cgo` CFLAGS/LDFLAGS explanation,
  the runtime `DYLD_LIBRARY_PATH`/`LD_LIBRARY_PATH` requirement (point at `target/<profile>`),
...
- [ ] **Step 1: Write `scripts/validate_bindings.sh`.** It seeds a temp pricing dir from the
  fixture, runs all three consumers over `bindings/testdata/claude-mini.jsonl`, extracts the raw
  `total_usd` token from each, and asserts all three are byte-identical:
...
export DYLD_LIBRARY_PATH="$LIBDIR" LD_LIBRARY_PATH="$LIBDIR" OBOL_LIB="$LIBDIR/$([ "$(uname)" = Darwin ] && echo libobol_ffi.dylib || echo libobol_ffi.so)"
...
# Rust CLI (JSON), extract total_usd token verbatim with python json (no reformatting of value text is fine: json.dumps of a float round-trips the same f64; we compare parsed repr).
rust_total=$($RUST run -q -p obol-cli -- estimate "$T" --dialect claude --json | python3 -c 'import sys,json;print(repr(json.load(sys.stdin)["total_usd"]))')
py_total=$(cd bindings/python && PYTHONPATH=. python3 -c "import obol,sys;print(repr(obol.estimate_path('$ROOT/$T', dialect='claude').total_usd))")
go_total=$(cd bindings/go && go run ./... 2>/dev/null || true)

echo "rust : $rust_total"
echo "py   : $py_total"

# Compare rust vs python (Go compared via a tiny helper program below if present)
if [ "$rust_total" != "$py_total" ]; then
  echo "MISMATCH rust($rust_total) != python($py_total)"; exit 1
fi
echo "OK: rust == python total_usd"
```

(Use `repr(float)` so both sides compare the exact same IEEE-754 value via Python's shortest
round-trip representation — Python parses both JSON tokens to the same `float`. For Go, add a
small `bindings/go/cmd/total/main.go` that prints `est.TotalUSD` with `strconv.FormatFloat(v,
'g', -1, 64)`, run it, and compare to the others by re-parsing all three through Python `float()`
before the string compare. Keep the comparison value-based, never format-based.)

- [ ] **Step 2: Make it executable and run it.** `chmod +x scripts/validate_bindings.sh && ./scripts/validate_bindings.sh`. Expected: `OK: rust == python total_usd` (and Go once wired).

- [ ] **Step 3: Write `docs/validation-ffi-2026-06-05.md`** documenting: what was validated
...
```bash
git add scripts/validate_bindings.sh docs/validation-ffi-2026-06-05.md bindings/go/cmd 2>/dev/null || true
...
- **Known soft spot:** Task 5 Step 5 flags that the cbindgen *CLI* (`gen-header.sh`) and the
  cbindgen *library* (drift test) must produce identical bytes; if they differ by version, make
  the committed header match the library output (the test is the gate). The implementer resolves
  this concretely during the task.
